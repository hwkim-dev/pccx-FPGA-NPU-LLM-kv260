...
def generate_softmax_frac_lut(filename="softmax_frac.mem"):
    print("Softmax 2^x (decimal part) 1024 division BRAM cheat sheet creation started...")

    NUM_ENTRIES = 1024

    # Hardware scaling factor (Q1.15 format)
    # Bump the number 1.0 into 32768 (2^15) in hardware!
    #Why? Since 2^0.999... is close to 2.0,
    # 2.0 * 32768 = 65536, which fills the 16-bit (0~65535) box with goosebumps!
    ...
    for i in range(NUM_ENTRIES):
        ...
        # 4. Clipping to 16-bit Unsigned range (0 to 65535) (prevent overflow)
    ...
    print("Now FPGA can chew up the e^x exponential function with just addition and shift!")
