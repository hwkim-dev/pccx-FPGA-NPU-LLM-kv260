...
def slice_weight_for_npu(weight_name, weight_matrix, block_size=64, save_dir="./npu_bram_tiles"):
    ...
    if pad_out > 0 or pad_in > 0:
        ...
        # Fill with zeros on the right and bottom using numpy.pad
    ...
    # Twist the shape so that it continues beautifully into a 64x64 chunk in memory (Numpy magic!)
    ...
    for y in range(grid_y):
        for x in range(grid_x):
            ...
            # We are currently in Phase 2 Python verification stage, so we will save it as .npy!
            file_name = f"{weight_name}_block_Y{y}_X{x}.npy"
            np.save(os.path.join(save_dir, file_name), tile_data)

    print(f"Slicing complete! Save location: {save_dir}/\n")
    return tiles

if __name__ == "__main__":
    # --- [Test Scenario] ---
    # Let's assume that this is Gemma 3N's Attention Query Projection weight!
    # Example: in_features = 2048, out_features = 2048
    # But for edge case testing, I'll put in a strange number (2000x1500) that doesn't fall to 64.
