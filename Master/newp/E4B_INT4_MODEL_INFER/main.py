...
def hw_prefetch(w, buf_idx):
    """If the next weight is an INT4 tuple, it is fetched secretly by a background thread."""
...
def hw_compute_pingpong(x, w, buf_idx, use_gelu=False):
    """Fire calculation with current buffer (buf_idx)! If it is not a tuple, bypass by regular matrix multiplication."""
    if ACCEL_MODE == "IGPU" and isinstance(w, tuple):
...
def rms_norm(x, gamma):
    ...
    # gamma weights are also safely guaranteed to be float32 contiguous arrays
...
def decode_logits(xs, altup_unprojs, W_final_norm, W_lm_head):
    ...
    # Immediately fire the x_final that has completed the calculation into the buffer number 0 that was loaded earlier!
...
def _sample(logits: np.ndarray, temperature: float, top_p: float,
    ...
    # Top-p sampling (this part can also be transferred to C++ later, but kept for now)
    if top_p < 1.0:
...
def main():
    ...
    TOP_P          = 0.9    
    REP_PENALTY    = 1.15   # Significantly increased from 1.02 to 1.15! (repeat loop absolute defense)
    ...
    # Entire conversation history (simple form)
    history_tokens = []
    ...
    print("\n--- Start a conversation (end: 'exit' or 'quit') ---")
    while True:
        try:
            user_input = input("\nUser: ")
            ...
            for _ in range(MAX_NEW_TOKENS):
                ...
                # Update the status of the printed text
