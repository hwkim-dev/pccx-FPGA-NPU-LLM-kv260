...
# ================================================================
# Maintain interface (only name left for legacy compatibility)
# ================================================================
def preload_and_free(W: dict, keys: list):
    print("[CPU_GEMV] Taichi removed. Ready for full operation mode with 6 C++ SIMD cores directly from RAM!")
    pass
...
def igpu_matmul_gelu(x_vec: np.ndarray, weight_data) -> np.ndarray:
    ...
    if isinstance(weight_data, tuple):
        ...
        # Python C_DLL GeLU call (already set on the CPU_CORE side, but manual call is omitted for convenience)
        import CPU_CORE
        return CPU_CORE.gelu(out)

def warmup():
    print("[CPU_GEMV] Multicore AVX2 SIMD engine warming up...")
