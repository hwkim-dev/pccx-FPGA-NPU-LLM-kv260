...
// Note: Single g_descriptorSet declaration has been deleted! (since it was declared as an array above)
...
extern "C"
{
    ...
    void run_vulkan_gemv_pingpong(const float *x, const float *scale, float *out, int M_out, int K_in, int buf_idx)
    {
        ...
        // Caution: Removed error with memcpy(out, g_outMapped...) here!
        ...
        // After completing the calculation, subtract the result from here only once!
        memcpy(out, g_outMapped, M_out * sizeof(float));

        vkFreeCommandBuffers(device, commandPool, 1, &commandBuffer);
    }

    // Legacy function left for compatibility (modified with array index [0])
    void run_vulkan_gemv(const float *x, const uint8_t *mat_p, const float *scale, float *out, int M_out, int K_in)
    {
        memcpy(g_xMapped, x, K_in * sizeof(float));
        memcpy(g_matMapped[0], mat_p, M_out * (K_in / 2) * sizeof(uint8_t)); // use [0]
        memcpy(g_scaleMapped, scale, M_out * sizeof(float));
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipeline);
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0, 1, &g_descriptorSet[0], 0, nullptr); // use [0]
