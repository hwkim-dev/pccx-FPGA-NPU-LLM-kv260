...
#List of layer large matrix keys that take the IGPU path
# free float16 original after preload_and_free loads these into VRAM
...
def forward_one_token(token_id, pos, W, W_embed, W_ple, norm_ple,
    ...
    for i in range(35):
        ...
        # W["W_q"][i] etc. is WeightProxy or float16 array
        ...
        xs_new[1:] += mapped   # Broadcast (maintain previous optimization)
...
def decode_logits(xs, altup_unprojs, W_final_norm, W_lm_head):
    ...
    # Calculate dots by lowering x_final to float16 → restore only the result to float32
    # Temporary memory: (vocab,) float16 = 512KB (extremely small compared to existing methods)
...
def main():
    ...
    # Upload all IGPU path weights to VRAM (INT16) and release the original float16
    # Effect: float16 array such as layers["W_q"][i] ~3.5GB → Replaced with WeightProxy (small object)
    # ============================================================
    print("[Memory] IGPU weighted VRAM pre-upload begins (float16 original to be released)...")
