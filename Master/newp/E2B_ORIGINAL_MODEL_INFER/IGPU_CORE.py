...
#
# float16 Load the original matrix into VRAM (INT16) and release it from RAM.
# Replace layers["W_q"][i], etc. with this object → float16 array is GCed.
# As long as there is a .shape, igpu_matmul operates normally.
# ================================================================
class WeightProxy:
    """Lightweight proxy used in place of the float16 original after VRAM upload is complete."""
...
# ================================================================
# VRAM cache: based on stable integer ID
#
# Problems with the existing id(weight_mat) method:
# - id() can be reused for other objects after freeing the float16 original
# - In that case, cache miss → reupload
#
# New method: Grant sequential integer stable_id when uploading → permanently unique
...
_next_stable_id = [0]          # mutable counter (bypass closure with list)
...
# 2. Replace the corresponding item in the layers dict with WeightProxy
...
def preload_and_free(W: dict, keys: list):
    ...
    for key in keys:
        ...
        proxies = []
        for w in W[key]:
            ti_mat, ti_scale = _get_or_upload_weight(w)   # Upload VRAM
            obj_id    = id(w)
            stable_id = _OBJID_TO_STABLE[obj_id]
            proxies.append(WeightProxy(stable_id, w.shape))
            uploaded += 1
        W[key] = proxies   # float16 array dereference → GC target
        print(f"[{key}] {len(proxies)} → VRAM transfer completed")

    gc.collect()
    print(f"[Memory] Total {uploaded}/{total} weight VRAM transfer & float original release ✓")
...
def warmup():
    ...
    print("[iGPU] JIT warm-up + INT16 NPU simulation mode running...")
