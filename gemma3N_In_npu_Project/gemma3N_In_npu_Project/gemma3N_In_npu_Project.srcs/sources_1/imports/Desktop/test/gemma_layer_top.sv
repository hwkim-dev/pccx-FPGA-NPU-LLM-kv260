...
module gemma_layer_top (
    ...
    // In reality, we will use the AXI-Stream (TVALID, TDATA, etc.) standard, but it is expressed conceptually!
    // -------------------------------------------------------------------
    ...
    input  logic [4:0]         i_result_sel,       // [Add] For selection of 32 channels
    output logic [1023:0]      o_npu_result_all,   // [Add] Full bus for DMA
    output logic               o_logic_anchor,     // [Special Move] Anchor to preserve all PE!
    output logic [15:0]        o_final_result      // Final result to be returned to CPU via DMA
);
    ...
    typedef enum logic [1:0] {
        ...
        ST_WAIT_RMS = 2'd1,  // Wait until mean_sq entered as 0x08 is converted to inverse square root
        ST_RUN      = 2'd2,  // Ping pong BRAM data bombardment with a 32x32 array!
        ST_WAIT_MAC = 2'd3   // Wait for completion of remaining calculations in Systolic pipeline (Wavefront)
    } state_t;
    ...
    // [Edit] Perfect port name matching and format casting
    ping_pong_bram #(
        .DATA_WIDTH(512),
        .ADDR_WIDTH(9)
    ) u_bram_token (
        ...
        .dma_write_data(i_dma_wdata_token), // {504'd0, ...} Delete and connect directly!        .npu_addr_a({3'd0, feed_counter}), // 6bit -> 9bit expansion
        .npu_addr_b(9'd0),                            // Not used
        .npu_read_data_a(flat_token_data),            // 512bit output
        .npu_read_data_b()                            // Not used
    );
    ...
    ping_pong_bram #(
        ...
        .dma_write_data(i_dma_wdata_weight), // {256'd0, ...} Delete and connect directly!        .npu_addr_a({3'd0, feed_counter});
        .npu_addr_b(9'd0),
    ...
    assign mac_valid_out = shift_reg_valid[63]; // After about 64 clocks, PE at the bottom right is completed.

    // // =========================================================================
    // 5. [Stage 3] Output Buffering & Activation MUX
    // =========================================================================
    // [Orthogonal method] Store 32-bit operation results of 32 channels at once in BRAM (prevent DSP pruning)
    logic [31:0] mac_result_row [0:31];
    always_comb begin
        for (int j = 0; j < 32; j++) begin
            // Extract data from the last row (Row 31) from the flat bus
    ...
    // [Positive method] Exposing the entire bus outside (for DMA preparation)
    assign o_npu_result_all = dma_result_bus;

    // [Special Move] Logical 'Anchor' to forcibly keep all PEs (1024) alive.
    // Even if only one of the 1024 bits changes, this value changes, so Vivado cannot delete the PE!
    ...
    // Keep the existing debug variable name as is, but burn the selected channel value
